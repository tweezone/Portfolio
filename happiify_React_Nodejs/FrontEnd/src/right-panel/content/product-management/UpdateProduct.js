...
class UpdateProduct extends Component{
    ...
    deleteOldImage=(index, id)=>{
        let { updateImages, deleteImageIds } = this.state;
        updateImages.splice(index,1);
        deleteImageIds.push(id);
        this.setState({ updateImages, deleteImageIds });
    }
    ...
    handleClickSave=(e)=>{
        ...
        const { model, price, taxClass_id, currency_id, quantity, weight, manufacturer_id, brand_id, date, status, 
                ordered, images, updateImages, uploadImages, deleteImageIds,name, desc, url, lang_id, category, 
                price_prefix } = this.state;
        ...
        if(!isP0_number.test(+price)){
            ...
            date===''? formData.append('date', '1970-01-01'):formData.append('date', date);
            
            uploadImages.forEach(one=> formData.append('uploadImages', one));

            axios.put(apiRoot + "products/update", formData, { 
                ...
                .catch(err=>{
                    console.log("[Error] - POST /products/add - at UpdateProduct component!");
                    console.log(err);
            });
            e.preventDefault();
        }
    ...
    componentDidMount(){
        ...
        axios.get(apiRoot + "products")
            .then((res) => { 
                const getOne=res.data.find(one=>one.products_id === productId);
                let date = getOne.products_date_available.slice(0, getOne.products_date_available.indexOf('T'));
                if(date === null){ date = '' }
                else if(date.includes('1900-01-01')){ date = ''; }
                this.setState({ 
                    model: getOne.products_model,
                    price: getOne.products_price,
                    taxClass_id: getOne.products_tax_class_id,
                    currency_id: getOne.currencies_id,
                    quantity: getOne.products_quantity,
                    weight: getOne.products_weight,
                    manufacturer_id: getOne.manufacturers_id,
                    brand_id: getOne.brands_id,
                    date,
                    status: getOne.products_status,
                    ordered: getOne.products_ordered,
                    name: getOne.products_name,
                    desc: getOne.products_description,
                    url: getOne.products_url,
                    lang_id: getOne.language_id
                });  
            })
            .catch((err) => {
                console.log("[Error] - GET /products - at UpdateProduct component!");
                console.log(err);         
        });        
        axios.get(apiRoot + "languages")
            ...
            .catch((err) => {
                console.log("[Error] - GET /languages - at UpdateProduct component!");
                console.log(err);           
        });
        axios.get(apiRoot + "products/tax_class")
            ...
            .catch((err) => {
                console.log("[Error] - GET /products/tax_class - at UpdateProduct component!");
                console.log(err);            
        });
        axios.get(apiRoot + "currencies")
            ...
            .catch((err) => { 
                console.log("[Error] - GET /currencies - at UpdateProduct component!");
                console.log(err);          
        });
        axios.get(apiRoot + "products/manufacturers")
            ...
            .catch((err) => { 
                console.log("[Error] - GET /products/manufacturers - at UpdateProduct component!");
                console.log(err);          
        });
        ...
        axios.get(apiRoot + "products/images/search?productId=" + productId)
            .then((res) => {
                this.setState({ updateImages: res.data });
            })
            .catch((err) => { 
                console.log("[Error] - GET /products/images/search?productId= - at UpdateProduct component!");
                console.log(err);          
        });
        axios.get(apiRoot + "products/categories")
            .then((res) => {
                const categoryList = res.data;
                axios.get(apiRoot + "products/categories/match")
                    .then((res) => {
                        const results=res.data.filter(one=>one.products_id === +productId);
                        let category=[];
                        results.forEach(one=>{
                    ...
                    .catch((err) => {
                        console.log("[Error] - GET /products/categories/match - at UpdateProduct component!");
                        console.log(err);       
                });
            })
            .catch((err) => {
                console.log("[Error] - GET /products/categories - at UpdateProduct component!");
                console.log(err);       
        });
        axios.get(apiRoot + "products/attributes")
            .then((res) => {
                const attributeDetail = res.data;
                const attributeList = this.getAttrArray(res.data);
                axios.get(apiRoot + "products/attributes/match")
                    .then((res) => {
                        const results=res.data.filter(one=>one.products_id === +productId);          
                        let price_prefix=[];
                        let attribute=[];
                        results.forEach(one=>{
                            let item = { 
                                id:one.products_options_values_to_products_options_id, 
                                p:one.options_id, 
                                ov:one.options_values_id, 
                                value:one.options_values_price, 
                                pre:one.price_prefix
                            };
                            price_prefix.push(item);
                            attribute.push(one.products_options_values_to_products_options_id);
                        })
                        this.setState({ attributeDetail, attributeList, price_prefix, attribute });
                    })
                    .catch((err) => {
                        console.log("[Error] - GET /products/attributes/match - at UpdateProduct component!");
                        console.log(err);    
                });
            })
            .catch((err) => {
                console.log("[Error] - GET /products/attributes - at UpdateProduct component!");
                console.log(err);    
        });
    ...
    render(){
        let lang='';
        if(this.props.langState ==='cn') lang=LANGUAGE.UpdateProduct.cn;
        else lang=LANGUAGE.UpdateProduct.en;
        ...
        const { model, price, taxClass_id, taxClassList, currency_id, currencyList, quantity, weight, 
                ...
                updateImages, imageUrls, name, desc, langList, categoryList, category, attributeDetail, attributeList, attribute, 
                url, lang_id, price_prefix } = this.state;
        
        // Handle images display
        let display=[];
        let count=0;
        updateImages.forEach((one,index)=>{
            display.push(
                <div className="card-deck border-warning mb-3" key={count++}> 
                    <div className='col'>
                        <div className="card border-secondary">
                            <a className="text-right mr-1" href="0" onClick={(e)=>{this.deleteOldImage(index, one.id); e.preventDefault()}}><span className='close'>&times;</span></a>
                            <img src={apiRoot + 'display/image/file?file=' + one.image} style={{width: '125px'}} alt={index}/>
                        </div>
                    </div>
                </div> 
            );
                ...
                <div className="card-deck border-warning mb-3" key={count++}> 
                    <div className='col'>
                        <div className="card border-secondary">
                            ...
                            <img className="card-img-top" src={one} style={{width: '125px'}} alt={index}/>
                        </div>
                    </div>
                </div> 
            );
        ...
        if(attribute.length !== 0){
            attribute.forEach((one, index)=>{
                ...
                const pre_item = price_prefix.find(o=>o.id === +one);
                rows.push(
                    <tr key={index} style={{verticalAlign: 'middle'}}>
                        ...
                        <td style={{verticalAlign: 'middle'}}>
                            <select className="form-control custom-select" style={{width: '60px'}} value={pre_item.pre} onChange={(e)=>this.handlePrefix(one, e)}>
                                <option key="1" value="+" >+</option> 
                                <option key="2" value="-">-</option>
                            </select>
                        </td>
                        <td style={{verticalAlign: 'middle'}} className="text-center">
                            <input name="variation" type="number" min="0" max={price} className="form-control" value={pre_item.value} onChange={(e)=>this.handleVariation(one, e)}/>
                        </td>
                    </tr>
                )
            });
            attribute_table=(
                <div className="row mt-2" style={{fontSize: "0.9em"}}>
