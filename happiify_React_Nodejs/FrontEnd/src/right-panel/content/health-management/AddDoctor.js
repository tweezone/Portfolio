...
*/
    ...
    handleClick=(e)=>{
        const { allCategories, langList, language, user_name, real_name, image, uploadImage, city_id, title, edu, quali, category, major, achiev } = this.state;
                ...
                axios.get(apiRoot + "health/doctors")
                    .then((res) => {
                        ...
                        if(user_name === ''){
                            ...
                            axios.post(apiRoot + "health/doctors/add", formData, { 
                            ...
                            .catch(err=>{ 
                                console.log("[Error] - GET /health/doctors/add - at AddDoctor component!");
                                console.log(err);
                            });
                        }
                    })
                    .catch((err) => { 
                        console.log("[Error] - GET /health/doctors - at AddDoctor component!");
                        console.log(err);           
                });
            })
            .catch((err) => { 
                console.log("[Error] - GET /users - at AddDoctor component!");
                console.log(err);            
        });
        e.preventDefault();
    }
    ...
    componentDidMount(){
        axios.get(apiRoot + "health/doctors/categories")
            ...
            .catch((err) => { 
                console.log("[Error] - GET /health/doctors/categories - at AddDoctor component!");
                console.log(err);            
            });
        ...
        axios.get(apiRoot + "languages")
            .then((res) => {          
                this.setState({ 
                    langList: res.data,
                    language: res.data[0].name
                });
            })
            .catch((err) => {  
                console.log("[Error] - GET /languages - at AddDoctor component!");
                console.log(err);            
        });
    }
                                    ...
                                    <div className="col col-sm-10 input-group-sm">
                                        <div className="row">
                                            <div className="col col-sm-10">
                                                <input name="image" type="file" className="form-control align-middle" id="image" value={this.state.image} onChange={this.handleUploadImage}/>
                                            </div>
                                            <div className="col col-sm-2 ">
                                                <div className="card border-secondary float-right">
                                                    { image === ''? 
                                                        <img className="card-img-top" style={{width: '130px'}} src="../../../images/default_user.png"  alt={'user icon'}/> :
                                                        <img className="card-img-top" style={{width: '130px'}} src={imageUrl} alt={'user icon'}/> }
                                                </div>
                                            </div>
                                        </div>
                                    </div>
