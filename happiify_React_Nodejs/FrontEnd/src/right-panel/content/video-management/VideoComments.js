...
class VideoComments extends Component{
    ...
    componentDidMount(){
        axios.get(apiRoot + "videos/comments")
            .then((res) => {            
                this.setState({ commentList: res.data });  
            })
            .catch((err) => {            
                console.log("Getting comments error in VideoComments component!");            
            });
    }

    handleClickDelete=(e)=>{
        axios.put(apiRoot + "videos/comments/delete", {id: e.target.value})
        .then(res => {
            axios.get(apiRoot + "videos/comments")
            .then((res) => {            
                this.setState({ commentList: res.data });  
            ...
            .catch((err) => {            
                console.log("Deleting comments error in VideoComments component!");            
            });
        })
        .catch(err=>{
    ...
    render(){
        let lang='';
        if(this.props.langState ==='cn') lang=LANGUAGE.VideoComments.cn;
        else lang=LANGUAGE.VideoComments.en;

        const row=[];
        let key=0;
        this.state.commentList.forEach((com)=>{
            if( (com.video_title.toUpperCase().indexOf(this.state.searchContent.toUpperCase())===-1) && 
                (com.username.toUpperCase().indexOf(this.state.searchContent.toUpperCase())===-1) &&
                (com.video_comments.toUpperCase().indexOf(this.state.searchContent.toUpperCase())===-1)) {
                return;
            }
            row.push(
                <tr key={key}>
                    ...
                    <td>{com.username}</td>
                    <td>{com.video_title}</td>
                    <td>{com.video_comments}</td>
